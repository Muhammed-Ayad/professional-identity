# Flutter & Serverpod Project: Professional Identity

**Professional Identity** is a full-stack digital professional identity and portfolio platform for developers, engineers, designers, and freelancers. It provides a verified, unified profile accessible through a public URL (`/u/:handle`) and QR code, replacing fragmented links and static CVs.

### Architecture Overview

- **Backend (`professional_identity_server`)**: Serverpod 4 with PostgreSQL ORM.
  - Authentication: Serverpod email IDP (`EmailIdpEndpoint`) and JWT token management.
  - Endpoints: `ProfileEndpoint` for profile management (`getMyProfile`, `getPublicProfile`, `isHandleAvailable`, `saveMyProfile`).
  - Database Models: `Profile` (table `profile`) linked to Serverpod `AuthUser` with cascade delete, unique handles, and public/private visibility.
- **Client (`professional_identity_client`)**: Generated client protocol, models, and endpoints.
- **Frontend (`professional_identity_flutter`)**: Flutter app with feature-first structure and **Hooks Riverpod**.
  - State Management: Hooks Riverpod (`authStateProvider`, `userProfileNotifierProvider`) with `HookConsumerWidget` and `ConsumerWidget`.
  - Repositories: `AuthRepository` and `ProfileRepository` encapsulating client communication.
  - UI/UX: Clean modern SaaS interface with `AppTheme` (light/dark mode), responsive cards, onboarding, profile editor dialog, and authenticated dashboard.

### Serverpod & Development Workflow
...
NEVER edit generated code. The server's `lib/src/generated/` directory and the whole `professional_identity_client` package are rewritten by the code generator. Change the `.spy.yaml` models, the endpoints, or `lib/server.dart` instead.
...
The app is launched from `professional_identity_flutter/lib/driver.dart`, which starts the Flutter driver extension with text entry emulation turned off so the app stays usable by hand. To let the driver type, set `enableTextEntryEmulation: true` there and `hot_restart` the app.
