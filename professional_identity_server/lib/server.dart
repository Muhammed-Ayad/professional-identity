...
import 'src/web/middleware/seo_middleware.dart';
...
void run(List<String> args) async {
  ...
  pod.initializeAuthServices(
    ...
    identityProviderBuilders: [
      ...
      ServerpodCloudEmailIdpConfig(
        appDisplayName: 'professional_identity',
      ),
    ],
  );
  ...
  // Setup minimal production health-check route.
  pod.webServer.addRoute(
    HealthRoute(),
    '/healthz',
  );

  // Setup SEO middleware for public profiles.
  // Registered as middleware so it intercepts /u/<handle>
  // requests before the FlutterRoute catch-all at '/' can serve the SPA
  // fallback. For valid public profiles, it serves server-rendered HTML
  // with Open Graph / Twitter Card metadata. For missing or private
  // profiles, it passes through to FlutterRoute.
